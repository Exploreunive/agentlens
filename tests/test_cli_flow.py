...
def test_cli_inbox_automatically_marks_forced_fixed_case_as_reopened_on_next_run(tmp_path: Path):
    work = tmp_path / 'proj'
    shutil.copytree(ROOT, work)
    ...
    rerun_inbox = subprocess.run(
        [sys.executable, 'cli.py', 'inbox', '--baseline', 'golden-run'],
        cwd=work,
        capture_output=True,
        text=True,
    )
    assert rerun_inbox.returncode == 0, rerun_inbox.stderr

    readme_text = (work / 'artifacts' / 'cases' / blocked_trace / 'README.md').read_text(encoding='utf-8')
    board_text = (work / 'artifacts' / 'cases' / 'index.html').read_text(encoding='utf-8')
    assert '- status: `fixed`' in readme_text
    assert '- workflow_state: `reopened`' in readme_text
    assert 'Reopened case' in board_text
